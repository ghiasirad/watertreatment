...
def updating_writer(a):
    ...
    s.send('{"request":"write","data":{"inputs":{"valve2_sp_out":'+repr(current_command)+'}}}\n')

    # import pdb; pdb.set_trace()
    #s.send('{"request":"read"}')
    data = json.loads(s.recv(1500))
    valve_2_out = int(data["state"]["valve_2_out"]*65535)
    level_2_peri = int(data["outputs"]["level_2_peri"]/3*65535)
    print data

    # import pdb; pdb.set_trace()
    context[slave_id].setValues(4, 1, [valve_2_out,level_2_peri])
...
def run_update_server():
    ...
    StartTcpServer(context, identity=identity, address=("192.168.95.26", 502))
