...
def updating_writer(a):
    ...
    s.send('{"request":"write","data":{"inputs":{"valve5_sp_in":'+repr(current_command)+'}}}\n')

    # import pdb; pdb.set_trace()
    #s.send('{"request":"read"}')
    data = json.loads(s.recv(1500))
    valve_5_in = int(data["state"]["valve_5_in"]*65535)
    level_5 = int(data["outputs"]["level_5"]/4*65535)
    print data

    # import pdb; pdb.set_trace()
    context[slave_id].setValues(4, 1, [valve_5_in,level_5])
...
def run_update_server():
    ...
    StartTcpServer(context, identity=identity, address=("192.168.95.31", 502))
