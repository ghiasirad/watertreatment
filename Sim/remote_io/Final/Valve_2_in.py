...
def updating_writer(a):
    ...
    s.send('{"request":"write","data":{"inputs":{"valve2_sp_in":'+repr(current_command)+'}}}\n')

    # import pdb; pdb.set_trace()
    #s.send('{"request":"read"}')
    data = json.loads(s.recv(1500))
    valve_2_in = int(data["state"]["valve_2_in"]*65535)
    level_2 = int(data["outputs"]["level_2"]/4*65535)
    print data

    # import pdb; pdb.set_trace()
    context[slave_id].setValues(4, 1, [valve_2_in,level_1])
...
def run_update_server():
    ...
    StartTcpServer(context, identity=identity, address=("192.168.95.25", 502))
