...
def updating_writer(a):
    ...
    s.send('{"request":"write","data":{"inputs":{"valve3_sp_out":'+repr(current_command)+'}}}\n')

    # import pdb; pdb.set_trace()
    #s.send('{"request":"read"}')
    data = json.loads(s.recv(1500))
    valve_3_out = int(data["state"]["valve_3_out"]*65535)
    level_3_peri = int(data["outputs"]["level_3_peri"]/3*65535)
    print data

    # import pdb; pdb.set_trace()
    context[slave_id].setValues(4, 1, [valve_3_out,level_3_peri])
...
def run_update_server():
    ...
    StartTcpServer(context, identity=identity, address=("192.168.95.28", 502))
