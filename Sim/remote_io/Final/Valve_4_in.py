...
def updating_writer(a):
    ...
    s.send('{"request":"write","data":{"inputs":{"valve4_sp_in":'+repr(current_command)+'}}}\n')

    # import pdb; pdb.set_trace()
    #s.send('{"request":"read"}')
    data = json.loads(s.recv(1500))
    valve_4_in = int(data["state"]["valve_4_in"]*65535)
    level_4 = int(data["outputs"]["level_4"]/4*65535)
    print data

    # import pdb; pdb.set_trace()
    context[slave_id].setValues(4, 1, [valve_4_in,level_4])
...
def run_update_server():
    ...
    StartTcpServer(context, identity=identity, address=("192.168.95.29", 502))
