...
def updating_writer(a):
    ...
    s.send('{"request":"write","data":{"inputs":{"valveMain_sp_out":'+repr(current_command)+'}}}\n')

    # import pdb; pdb.set_trace()
    #s.send('{"request":"read"}')
    data = json.loads(s.recv(1500))
    valve_out = int(data["state"]["valve_out"]*65535)
    water_out = int(data["state"]["water_out"]/10.0*65535)
    # print data

    # import pdb; pdb.set_trace()
    context[slave_id].setValues(4, 1, [valve_out,water_out])
...
def run_update_server():
    ...
    StartTcpServer(context, identity=identity, address=("192.168.95.22", 502))
