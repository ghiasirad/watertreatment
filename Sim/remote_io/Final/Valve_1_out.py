...
def updating_writer(a):
    ...
    s.send('{"request":"write","data":{"inputs":{"valve1_sp_out":'+repr(current_command)+'}}}\n')

    # import pdb; pdb.set_trace()
    #s.send('{"request":"read"}')
    data = json.loads(s.recv(1500))
    valve_1_out = int(data["state"]["valve_1_out"]*65535)
    level_1_peri = int(data["outputs"]["level_1_peri"]/3*65535)
    print data

    # import pdb; pdb.set_trace()
    context[slave_id].setValues(4, 1, [valve_1_out,level_1_peri])
...
def run_update_server():
    ...
    StartTcpServer(context, identity=identity, address=("192.168.95.24", 502))
