...
void WT::set_initial_values(Json::Value inputs) {
    ...
    // Main Valves
    if (inputs["inputs"].isMember("valveMain_sp_in")) {
        value_valve_in = inputs["inputs"]["valveMain_sp_in"].asDouble();
    }
    if (inputs["inputs"].isMember("valveMain_sp_out")) {
        value_valve_out = inputs["inputs"]["valveMain_sp_out"].asDouble();
    }
    if (inputs["inputs"].isMember("valveMain_sp_chem")) {
        value_chem_in = inputs["inputs"]["valveMain_sp_chem"].asDouble();
    }

    // Other Valves
    if (inputs["inputs"].isMember("valve1_sp_in")) {
        value_valve_1_in = inputs["inputs"]["valve1_sp_in"].asDouble();
    }
    if (inputs["inputs"].isMember("valve1_sp_out")) {
        value_valve_1_out = inputs["inputs"]["valve1_sp_out"].asDouble();
    }

    if (inputs["inputs"].isMember("valve2_sp_in")) {
        value_valve_2_in = inputs["inputs"]["valve2_sp_in"].asDouble();
    }
    if (inputs["inputs"].isMember("valve2_sp_out")) {
        value_valve_2_out = inputs["inputs"]["valve2_sp_out"].asDouble();
    }

    if (inputs["inputs"].isMember("valve3_sp_in")) {
        value_valve_3_in = inputs["inputs"]["valve3_sp_in"].asDouble();
    }
    if (inputs["inputs"].isMember("valve3_sp_out")) {
        value_valve_3_out = inputs["inputs"]["valve3_sp_out"].asDouble();
    }

    if (inputs["inputs"].isMember("valve4_sp_in")) {
        value_valve_4_in = inputs["inputs"]["valve4_sp_in"].asDouble();
    }
    if (inputs["inputs"].isMember("valve4_sp_out")) {
        value_valve_4_out = inputs["inputs"]["valve4_sp_out"].asDouble();
    }

    if (inputs["inputs"].isMember("valve5_sp_in")) {
        value_valve_5_in = inputs["inputs"]["valve5_sp_in"].asDouble();
    }
    if (inputs["inputs"].isMember("valve5_sp_out")) {
        value_valve_5_out = inputs["inputs"]["valve5_sp_out"].asDouble();
    }

    if (inputs["inputs"].isMember("valve6_sp_in")) {
        value_valve_6_in = inputs["inputs"]["valve6_sp_in"].asDouble();
    }
    if (inputs["inputs"].isMember("valve6_sp_out")) {
        value_valve_6_out = inputs["inputs"]["valve6_sp_out"].asDouble();
    }
...
Json::Value WT::get_state_json() {
    ...
    state["state"]["water_in"] = water_in_display;
    state["state"]["water_out"] = water_out_display;

    state["state"]["valve_1_in"] = valvesArray_in[0];
    state["state"]["valve_2_in"] = valvesArray_in[1];
    state["state"]["valve_3_in"] = valvesArray_in[2];
    state["state"]["valve_4_in"] = valvesArray_in[3];
    state["state"]["valve_5_in"] = valvesArray_in[4];
    state["state"]["valve_6_in"] = valvesArray_in[5];
    state["state"]["valve_in"] = valvesArray_in[6];

    state["state"]["valve_1_out"] = valvesArray_out[0];
    state["state"]["valve_2_out"] = valvesArray_out[1];
    state["state"]["valve_3_out"] = valvesArray_out[2];
    state["state"]["valve_4_out"] = valvesArray_out[3];
    state["state"]["valve_5_out"] = valvesArray_out[4];
    state["state"]["valve_6_out"] = valvesArray_out[5];
    state["state"]["valve_out"] = valvesArray_out[6];
...
void WT::process_values(){
    ...
    water_in_display = water_in*valvesArray_in[6];
    water_out_display = water_out*valvesArray_out[6];
...
double WT::change_level_container(int i){ // Change in level_peri per stepSize (1 second)
	...
	if (flowInside == 0 && levelsArray[i] <= containerHight && water_in*valvesArray_in[6] > 0 && levelsArray[6] > 0){
		...
		changeLevel = valvesArray_in[i]*valveFlowLimit/containerBase_in;
